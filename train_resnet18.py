...
def train_resnet18(epochs=50, save_interval=5):
    resnet_directory = 'resnet18_results'
    if not os.path.exists(resnet_directory):
        os.makedirs(resnet_directory)
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % save_interval == 0:
            model_path = os.path.join(resnet_directory, f'resnet18_epoch{epoch+1}.pth')
            save_model(model, model_path)
            plot_loss(train_losses, val_losses, epoch, resnet_directory)

    final_model_path = os.path.join(resnet_directory, 'resnet18_final.pth')
